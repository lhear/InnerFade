...
func (c *Conn) makeClientHello() (*clientHelloMsg, *keySharePrivateKeys, *echClientContext, error) {
	...
	var err error
	// Generate random bytes based on configuration
...
func (c *Conn) clientHandshake(ctx context.Context) (err error) {
	...
	// Check for downgrade canaries, considering custom random usage
	var tls12Downgrade, tls11Downgrade bool
	tls12Downgrade = len(serverHello.random) >= 32 && string(serverHello.random[24:]) == downgradeCanaryTLS12
	tls11Downgrade = len(serverHello.random) >= 32 && string(serverHello.random[24:]) == downgradeCanaryTLS11
